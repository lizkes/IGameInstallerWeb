...
type ResourceInstallCodeData = {
  install_code: string;
  trade_id: number;
  remain_coin?: number;
};
const useResourceInstallCodeMutation = (
  options?: ApiMutationOptions<
    ResourceInstallCodeData,
    {
      resourceId: number;
    }
  >
  ...
  return useMutation<
    AxiosResponse<ResourceInstallCodeData, any>,
    AxiosError,
    {
      resourceId: number;
    }
  >(
    ...
    async ({ resourceId }) => {
      const authAxios = await getAuthAxios();
      return authAxios.get(`/resource/${resourceId}/install_code`);
    },
    options
  );
};
...
export {
  ...
  useResourceInstallCodeMutation,
};
